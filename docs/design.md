       ...
       └─ ctx.register_hook("on_session_start", _on_session_start)
            └─ _on_session_start()
                 └─ _restore_active_task()
                      └─ Read ~/.hermes/sag_tasks/.active_task → set _active_task_id
...
**Summary generation:** On-demand via LLM at prefetch time (方案 B). Summaries are cached in `task_state.json` until the next `sag_task_advance`.
...
Each task has its own Git repository at `~/.hermes/sag_tasks/<task_id>/.git/`, pushed to `git@github.com:charlenchen/<task_id>.git`.
...
| Item | Value |
|-------|-------|
| Plugin directory (gitfile) | `~/.hermes/plugins/sagtask/` → gitdir: `~/.hermes/sag_tasks/sagtask-devop/src/.git` |
| Dev source (canonical git) | `~/.hermes/sag_tasks/sagtask-devop/src/sagtask/` |
| Plugin name | `sagtask` |
